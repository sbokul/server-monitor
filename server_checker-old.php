...
function alert($host, $server_name, $provider_email) {
    ...
    $email_from = 'donot_reply@carbontsb.com>';
    $to = 'carbontsb@gmail.com';
    $subject = $server_name.' ( '.$host.' Your network 100% Timed out )';

    $message = $server_name.' | '.$host.' | Your network 100% Timed out, pls check and revert to me
Regards
NOC
www.carbontsb.com';
    ...
    $result = $mg->sendMessage($domain, array('from' => 'Service Notification <donot_reply@carbontsb.com>',
        'to' => $to,
        'subject' => $subject,
        'text' => $message));
    if(!empty($provider_email)) {
        $message2 = "Your network 100% Timed out, pls check and revert to me \nRegards \nNOC \nwww.carbontsb.com";
        $result2 = $mg->sendMessage($domain, array('from' => 'Service Notification <donot_reply@carbontsb.com>',
            'to' => $provider_email,
            'subject' => $subject,
            'text' => $message2));
    }
